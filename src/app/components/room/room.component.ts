...
export class RoomComponent implements OnInit, OnDestroy {
	...
	public get previousGames(): Game[] {return this._previousGames;}
	...
	ngOnInit() {
		this.route.params.subscribe(params => {
			...
			if(this._roomId) {
				...
				let previousGamesSubscription: Subscription = null;
				this.roomSubscription = this.roomsService.observeRoom(this._roomId).subscribe(roomMessage => {
					...
					if(previousGamesSubscription) {
						previousGamesSubscription.unsubscribe();
					}

					if(this.room.previousGamesIds.length > 0) {
						previousGamesSubscription = this.gamesService
							.observePreviousGames(this._roomId)
							.pipe(map(message => message.content.previousGames))
							.subscribe((previousGames) => {
								this._previousGames = previousGames;
							})
					} else {
						previousGamesSubscription = null;
					}
				});
			}
		});
